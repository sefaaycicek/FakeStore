 ...
 * @author saycicek
 */
class GetBasketProductsUseCaseTest {

    private lateinit var repository: FakeStoreRepository
    private lateinit var sut: GetBasketProductsUseCase

    @Before
    fun setup() {
        repository = mockk(relaxed = true)
        sut = GetBasketProductsUseCase(repository)
    }

    @Test
    fun `invoke getBasketProducts fun in FakeStoreRepository when execute the use case`() =
        runTest {
            //when
            sut.invoke().onCompletion {
                coVerify(exactly = 1) { repository.getBasketProducts() }
            }
        }
    ...
    fun `emits IResult Loading as first value when execute the use case`() = runTest {
        ...
        //when
        coEvery { repository.getBasketProducts() }
    ...
    fun `emits IResult Error when execute the use case when throws an exception`() = runTest {
        ...
        //when
        coEvery { repository.getBasketProducts() } throws IOException()
        val result = sut.invoke()
    ...
    fun `emits IResult Success when execute the use case without problem`() = runTest {
        ...
        //when
        coEvery { repository.getBasketProducts() } returns listOf()
        val result = sut.invoke()
