...
class SearchProductUseCaseTest {
    ...
    private lateinit var sut: SearchProductUseCase
    ...
    fun setup() {
        ...
        sut = SearchProductUseCase(repository)
    }

    @Test
    fun `invoke searchProduct fun in FakeStoreRepository when execute the use case`() =
        runTest {
            //when
            sut.invoke(query, limit, skip).onCompletion {
                coVerify(exactly = 1) { repository.searchProduct(any(), any(), any()) }
            }
        }
    ...
    fun `emits IResult Loading as first value when execute the use case`() = runTest {
        ...
        //when
        coEvery { repository.searchProduct(any<String>(), any<Int>(), any<Int>()) }
        val result = sut.invoke(query, limit, skip)
    ...
    fun `emits IResult Error when execute the use case when throws an exception`() = runTest {
        ...
        //when
        coEvery { repository.searchProduct(any<String>(), any<Int>(), any<Int>()) } throws IOException()
        val result = sut.invoke(query, limit, skip)
    ...
    fun `emits IResult Success when execute the use case without problem`() = runTest {
        ...
        //when
        coEvery { repository.searchProduct(any<String>(), any<Int>(), any<Int>()) } returns productsResponse
        val result = sut.invoke(query, limit, skip)
