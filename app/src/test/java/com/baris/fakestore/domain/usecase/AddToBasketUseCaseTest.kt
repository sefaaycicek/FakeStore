...
class AddToBasketUseCaseTest {
    ...
    private lateinit var sut: AddToBasketUseCase
    ...
    fun setup() {
        ...
        sut = AddToBasketUseCase(repository)
    }

    @Test
    fun `invoke addToBasket fun in FakeStoreRepository when execute the use case`() =
        runTest {
            //when
            sut.invoke(productId).onCompletion {
                coVerify(exactly = 1) { repository.addToBasket(any()) }
            }
        }
    ...
    fun `emits IResult Loading as first value when execute the use case`() = runTest {
        ...
        //when
        coEvery { repository.addToBasket(any<Int>()) }
    ...
    fun `emits IResult Error when execute the use case when throws an exception`() = runTest {
        ...
        //when
        coEvery { repository.addToBasket(any<Int>()) } throws IOException()
        val result = sut.invoke(productId)
    ...
    fun `emits IResult Success when execute the use case without problem`() = runTest {
        ...
        //when
        coEvery { repository.addToBasket(any<Int>()) } returns Unit
        val result = sut.invoke(productId)
